...
usage() { echo "Usage: $0 [-g <genome>] [-f <gtf>] [-t <threads>] [-p <prefix_seperator>] [-s <offset_suffix>] [-b <bam_suffix>] [-o <output_dir>] [-i <input_dir>] [-d <proceed_with_ORFCalling>]" 1>&2; exit 0; }

while getopts ":g:f:t:p:s:b:o:i:d:" arg; do
    case "${arg}" in
        g)
            g=${OPTARG}
            ;;
